...
class OurWordFilterPlugin
{
  ...
  function ourMenu()
  {
    ...
    //called from ourMenu function to load assets
  ...
  //Enter words to be filtered ko HTML called in add_submenu_page
  function wordFilterPage()
  { ?>
    <div class="wrap">
      <h1>Word Filter</h1>
      <?php if ($_POST['justsubmitted'] == "true")
        $this->handleForm() ?>
        <form method="POST">
...
//update option for enter words to be filtered
function handleForm()
{
...
//Settings for another part, what to display in filtered text
  function ourSettings()
    ...
    //HTML called from add_settings_field
  ...
  //Enter replacement word ko section ko HTML (coming from add_submenu_page not form handling)
  ...
  { ?>
    <div class="wrap">
      ...
      <form action="options.php" method="POST">
        <?php
        settings_errors(); //handles error msgs
        settings_fields('replacementFields'); //handles nonce, actions, security and permissions for us.
        do_settings_sections('word-filter-options'); //actual HTML from sections
        submit_button();
        ?>
      </form>
    </div>
  ...
  //Logic to filter out words called on top
